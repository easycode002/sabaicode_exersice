...
export class Library {
    ...
    removeBook(iD) {
        let isRemoved = false;
        const indexToRemove = this.books.findIndex(b => b.isbn === iD || b.title == iD)
        if (indexToRemove !== -1) {
    ...
    updateBook(isbns, updatedBookData) {
        ...
        const indexToUpdate = this.books.findIndex(book => book.isbn === isbn || book.title === title || book.author === author);
        if (indexToUpdate !== -1) {
            this.books[indexToUpdate] = { ...this.books[indexToUpdate], ...updatedBookData };
    ...
    borrowBook(isbn) {
        ...
        if (bookIndex !== -1) {
            if (this.books[bookIndex].available) {
                ...
                console.log(`Book "${this.books[bookIndex].title}" with ISBN ${isbn} has been borrowed.`);
            } else {
                console.log(`Book "${this.books[bookIndex].title}" with ISBN ${isbn} is not available.`);
            }
        } else {
            console.log(`Book with ISBN ${isbn} not found in the library.`);
        }
    ...
    returnBook(isbn) {
        ...
        if (bookIndex !== -1) {
            if (!this.books[bookIndex].available) {
                ...
                console.log(`Book "${this.books[bookIndex].title}" with ISBN ${isbn} has been returned.`);
            } else {
                console.log(`Book "${this.books[bookIndex].title}" with ISBN ${isbn} is already available.`);
            }
        } else {
            console.log(`Book with ISBN ${isbn} not found in the library.`);
        }
...
// // search book
// let handleSearch = { title: "Title03", author: "Author03" };
