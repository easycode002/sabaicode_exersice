...
export class Library {
    ...
    removBook(userInput) {
        const indexToremove = this.#books.findIndex(book => book.getISBN() === userInput || book.getTitle() === userInput);
        /* Way001 ====================
        if (indexToremove !== -1) {
    ...
    */
    borrowBook(inputISBN) {
        const indexBook = this.#books.findIndex(book => book.getISBN() === inputISBN)
        // console.log(`File`,indexBook);   
        if (indexBook !== -1) {
            const bookToBorrow = this.#books[indexBook];
            if (bookToBorrow.isAvailable()) {
                bookToBorrow.setAvailable(false)
                console.log(`Book ${this.#books[indexBook].getTitle()} ID:${inputISBN} was borrowed!`)
            } else {
                console.log(`Book ${this.#books[indexBook].getTitle()} ID:${inputISBN} not found!!!`);
            }
        } else {
    ...
    */
    returnBooks(inputTitle) {
        const indexBook = this.#books.findIndex(book => book.getTitle().toLowerCase() === inputTitle.toLowerCase())
        if (indexBook !== -1) {
            const isBorrowBook = this.#books[indexBook]
            if (!isBorrowBook.isAvailable()) {
                isBorrowBook.setAvailable(true);
                console.log(`Book ${this.#books[indexBook].getTitle()} - ID:${inputTitle} returned!`);
            } else {
                console.log(`Book ${this.#books[indexBook].getTitle()} - ID:${inputTitle} not return!!!`);
            }
        } else {
